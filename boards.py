...
def get_threads(id):
    # Tässä kyselyssä haetaan ketjun id, nimi, ketjun luojan käyttäjänimi, jokaisen ketjun viimeisimmän vastauksen aika ja ketjun vastausten määrä
    # ja järjestetään ketjut viimeisimmän vastauksen perusteella
    sql = "SELECT t.id, t.title, u.username, t.created_at, MAX(c.created_at), (SELECT COUNT(*) FROM comments WHERE t.id=thread_id) " \
          "FROM threads t LEFT JOIN users u ON t.user_id=u.id LEFT JOIN comments c ON t.id=c.thread_id WHERE t.board_id=:id " \
          "GROUP BY t.id, u.username ORDER BY max DESC"
    result = db.session.execute(sql, {"id": id})
    return result.fetchall()
...
def create_thread(title, content, board_id):
    ...
    result = db.session.execute(
        sql, {"board_id": board_id, "user_id": user_id, "title": title})
...
def create_board(boardname, secret):
    try:
        ...
        db.session.execute(sql, {"boardname": boardname, "secret": secret})
        db.session.commit()
        return True
    except:
        return False
...
def secret_board_access(board_id):
    ...
    result = db.session.execute(
        sql, {"board_id": board_id, "user_id": user_id})
